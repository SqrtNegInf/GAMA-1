!   Fortran
!u# https://rosettacode.org/wiki/Knapsack_problem/0-1
!c# 2024-01-28 
...
! INITIALIZE.
          ...
          first_loop: do ll = 1 , n
                      ...
                      if( chs==0 )then
                          ...
                          a = w(ll + 1) - chs
                          b = ip + p(ll + 1)
                          lim1 = int ( b - a*float(p(ll))/float(w(ll)) )
...
!   call system_clock(count=xx)
    call start_knapsack(n,profit(1:22),p(1:22),capacity,result_val,members)
